...
describe("prefetch helpers", () => {
  ...
  // products.recommendations() unwraps res.data
  it("prefetchProductRecommendations populates query cache", async () => {
    const recommendations = [{ uuid: "pkg-1", name: "Bundle A" }];
    mockApiResponse({ data: recommendations });

    const qc = new QueryClient();
    await prefetchProductRecommendations(qc, client, "test-slug");

    const cached = qc.getQueryData(["spaceis", "product-recommendations", "test-slug"]);
  ...
  it("prefetchTopCustomers populates query cache", async () => {
    const customers = [{ username: "Steve", total_spent: 5000 }];
  ...
  it("prefetchLatestOrders populates query cache", async () => {
    const orders = [{ uuid: "order-1", username: "Steve" }];
  ...
  it("prefetchAgreements populates query cache", async () => {
    const agreements = [{ uuid: "1", name: "Terms of Service" }];
