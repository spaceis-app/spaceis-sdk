...
<script>
(function() {
    var paymentMethods = <?= json_encode($paymentMethods, JSON_UNESCAPED_UNICODE) ?>;
    var agreements = <?= json_encode($agreements, JSON_UNESCAPED_UNICODE) ?>;
    var selectedMethodUuid = paymentMethods.length > 0 ? paymentMethods[0].uuid : null;
    var checkedAgreements = {};
    var nick = '';
    var email = '';
    var discountCode = '';
    var placing = false;
    ...
    function render() {
        ...
        var commission = selectedMethod ? (selectedMethod.commission || 0) : 0;
        var commissionAmount = commission > 0 ? Math.round((finalPrice * commission) / 100) : 0;
        var totalWithCommission = finalPrice + commissionAmount;
        ...
        items.forEach(function(item) {
            ...
            html += '<div class="checkout-item-name">' + esc(item.shop_product ? item.shop_product.name : '') + '</div>';
            if (showVariant) {
        ...
        if (paymentMethods.length === 0) {
            ...
            paymentMethods.forEach(function(m) {
                var sel = selectedMethodUuid === m.uuid;
        ...
        if (agreements.length > 0) {
            ...
            agreements.forEach(function(a) {
                var checked = !!checkedAgreements[a.uuid];
    ...
    window.checkoutState = {
        ...
        selectMethod: function(uuid) {
            selectedMethodUuid = uuid;
            render();
        },
        toggleAgreement: function(uuid, checked) {
            if (checked) checkedAgreements[uuid] = true;
            else delete checkedAgreements[uuid];
        },
        placeOrder: function() {
            // Read current values from inputs
            var nickInput = document.getElementById('checkout-nick');
            ...
            if (errors.length > 0) {
                errors.forEach(function(e){SpaceISApp.showToast(e,'error')});
                return;
            }
            ...
            var agreementUuids = Object.keys(checkedAgreements);

            // Execute recaptcha, then place order
            client.recaptcha.execute('checkout').catch(function() {
                return '';
            }).then(function(token) {
